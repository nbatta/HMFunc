...
class Cosmology:
    ...
    def __init__(
        ...
        try:
            self.num_mnu = cosmo['num_massive_neutrinos']
        except KeyError:
            self.num_mnu = defaultCosmology['num_massive_neutrinos']
            warnings.warn("No number of massive neutrinos specified; assuming default of "+str(self.num_mnu))
        ...
        self.pars.set_cosmology(H0=H0, cosmomc_theta=theta, ombh2=self.ombh2, omch2=self.omch2,
                                mnu=self.mnu, tau=self.tau, nnu=self.nnu, num_massive_neutrinos=self.num_mnu)
        self.pars.Reion.Reionization = 0
